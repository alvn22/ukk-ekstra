...
<head>
    ...
    <?php 
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            $qc = "SELECT * FROM absensi WHERE id_ekstra = '$idekstra'";
            $r = mysqli_query($con,$qc);
            $row = mysqli_fetch_assoc($r);

            if(!empty($row['id_ekstra'])){
                alert('error','Hanya dapat membuat 1 absensi!');
            } else {
                if(isset($_POST['submit'])){
                    $tanggal = $_POST['tanggal'];
        
                    $q = "INSERT INTO absensi (id_ekstra, id_user, tanggal) VALUES ('$idekstra','$id','$tanggal')";
                    $r = mysqli_query($con,$q);
                    
                    alert('success','Berhasil menambahkan absensi');    
                }
            }
        }

        // $q = "DELETE FROM absensi WHERE tanggal < NOW() - INTERVAL 1 DAY";
        // mysqli_query($con,$q);

        if (isset($_GET["action"]) && $_GET["action"] == "delete" && isset($_GET["id"])) {
            $id = $_GET["id"];
            $query = "DELETE FROM absensi WHERE id_buat_absensi=$id";
            $result = mysqli_query($con, $query);
            alert('success','Berhasil menghapus absensi');
        }
    ...
    <title>Pelatih - Buat Absensi</title>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-2">
                        <div class=""></div>
                        <button class="btn btn-primary" data-toggle="modal" data-target="#absenModal">
                                    ...
                                    <div class="modal-body">
                                        <div class="row">
                                            <div class="form-group col-md-10 mb-3">
                                                ...
                                                <?php
                                                    $id = $_SESSION["id_user"];
                                                    $r = $con->query("SELECT nama_ekstra FROM ekstra JOIN pelatih_ekstra ON ekstra.id_ekstra = pelatih_ekstra.id_ekstra WHERE id_user=$id");
                                                    $nm = $r->fetch_assoc();
                                                ?>
                                                <input type="text" name="ekstra" value="<?php echo $nm['nama_ekstra'] ?>" class="form-control" readonly>
                                            </div>
                                            <div class="col-md-10 mb-3">
                                                <label>Tanggal:</label>
                                                <input type="date" value="<?php echo date('Y-m-d') ?>" name="tanggal" class="form-control" readonly>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <tbody>
                                <?php
                                    $sql = "SELECT * FROM absensi a 
                                        JOIN pelatih_ekstra p ON a.id_user = p.id_user 
                                        JOIN ekstra e ON a.id_ekstra = e.id_ekstra WHERE p.id_user = $id";
                                    ...
                                    if($res){
                                        while($data = mysqli_fetch_assoc($res)){
                                            ...
                                            echo<<<res
                                                ...
                                                <td>$tanggal</td>
                                                <td><a href="buat_absensi.php?action=delete&id=$data[id_buat_absensi]" type="button" name="delete" class="btn btn-sm btn-danger">Hapus</a></td>
                                                </tr>
                                            res;
                                            $i++;
                                        }
                                ...
                                <td>Voli</td>
                                <td>Kamis</td>
                                <td><a href="" type="button" name="delete" class="btn btn-sm btn-danger">Hapus</a></td>
                                </tr> -->
                            </tbody>
                        </table>
                    </div>
                    <!-- <a href="" type="button" name="delete" class="btn btn-sm btn-warning mr-2">Edit</a> -->
